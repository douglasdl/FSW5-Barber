...
async function seedDatabase2() {
  try {
    ...
    const hirakataBarbershops = [
      {
        name: "Barber Shop Nishijima",
        address: "3 Chome-4-7 Kuzuhaasahi, Hirakata, Osaka 573-1111",
        imageUrl: "https://lh5.googleusercontent.com/p/AF1QipPLcrRCJ4SfROl9FeM77yIsh-FwwjsgjK-hJ3xX",
        phones: ["(072) 856-1737"],
        description:
          `
          domingo
    ...
    const barbershops = [];
    for (let i = 0; i < hirakataBarbershops.length; i++) {
      const barbershop = await prisma.barbershop.create({
        data: {
          name: hirakataBarbershops[i].name,
          address: hirakataBarbershops[i].address,
          imageUrl: hirakataBarbershops[i].imageUrl,
          phones: hirakataBarbershops[i].phones,
          description: hirakataBarbershops[i].description,
        },
      });
